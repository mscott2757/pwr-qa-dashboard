...
class Test < ApplicationRecord
  ...
  def self.edit_all_as_json
    Test.all.includes(:primary_app, :environment_tag, :test_type, :application_tags).as_json(only: [:name, :id, :parameterized, :group], include: { primary_app: { only: [:name, :id] }, test_type: {only: [:name, :id] }, application_tags: { only: [:name, :id] }, environment_tag: { only: [:name, :id] } }).uniq{ |test| test["name"] }.sort_by { |test| test["name"].downcase }
  end
